...
export async function getReliableImageUrl(imagePath: string): Promise<string> {
  ...
  try {
    ...
    // Special case for blog/filename.jpg format - may need specific handling
    if (imagePath.startsWith("blog/")) {
      console.log("Detected 'blog/' prefix, using blog-images bucket first");
      try {
        // Extract the filename from blog/filename.jpg
        const objectPath = imagePath.split("/").slice(1).join("/");

        // Try with blog-images bucket first
        ...
        // If direct path fails, try without 'blog/' prefix in blog-images bucket
        ...
        console.error("Error with special case handling:", err);
      }
    }
    ...
    try {
      ...
      // If error, try alternative buckets
